...
class TumorStromaRatioHeatMapper:
    ...
    def __init__(self, parameters):
        ...
        # self.radius_in_pixels = self.calc_radius_px(self.hotspot_area, self.image_spacing)
        self.img = None
        ...
        # print('Bulk mask level: {}, classification mask level: {}'.format(self.bulk_mask_level, self.classification_map_level ))
    ...
    def build_circle_mask(self):
        ...
        xv, yv = np.meshgrid(np.linspace(-radius, radius, 2 * radius + 1), np.linspace(-radius, radius, 2 * radius + 1))
    ...
    def load_wsis(self):
        ...
        """
        # print(f'Image filename: {self.img_filename}')
        self.img = wsireader.open(self.img_filename)
        
        # print('Classification filename :{}'.format(self.classification_map_filename))
        self.tissue_mask = wsireader.open(self.tissue_mask_filename)
        if self.bulk_mask_filename is not None:
            # print('Bulk mask filename = {}'.format(self.bulk_mask_filename))
        ...
        if os.path.isfile(self.classification_map_filename) == True:
    ...
    def compute_ratios(self):
        ...
        # print('TISSUE MASK DIMS',tissue_mask_dims)
        classification_map_dims = self.classification_map.getDimensions()
        # print('CLASSIFICATION MASK DIMS',classification_map_dims)
        if self.bulk_mask:
            bulk_mask_dims = self.bulk_mask.getDimensions()
            # print('Bulk Mask Dims: {}'.format(bulk_mask_dims))
    ...
    def extract_numpy_arrays(self):
        ...
        self.classification_map_np = self.classification_map.getUCharPatch(0, 0, dims_map_selected_level[0], dims_map_selected_level[1], self.classification_map_level).squeeze()
        # extract tissue mask in numpy format
        tissue_mask_dims_at_selected_level = self.tissue_mask.getLevelDimensions(self.tissue_mask_level)
        self.tissue_mask_np = self.tissue_mask.getUCharPatch(0, 0, tissue_mask_dims_at_selected_level[0], tissue_mask_dims_at_selected_level[1], self.tissue_mask_level).squeeze()
        if self.bulk_mask:
            bulk_mask_dims_at_selected_level = self.bulk_mask.getLevelDimensions(self.bulk_mask_level)
            self.bulk_mask_np = self.bulk_mask.getUCharPatch(0, 0, bulk_mask_dims_at_selected_level[0], bulk_mask_dims_at_selected_level[1], self.bulk_mask_level).squeeze()
    ...
    def get_top_hottest_regions(self, output_path):
        ...
        while len(max_values) < self.number_of_hotspots:
            ...
            nonmaxsur = cv2.circle(nonmaxsur, (tmp_x, tmp_y), int(self.radius_in_pixels), 0, -1) # we want to block out all values in the circle, so let's remove them color = 0, thickness = -1 to fill 
    ...
    def compute_circle_diameter_px(self):
        ...
        self.pixel_size_selected_level = self.pixel_size_level_0 * self.img.getDimensions()[0] / self.img.getLevelDimensions(self.level)[0]
    ...
    def compute_tsr_wsi(self):
        ...
        #Get what is stroma contained in the tumour
        stroma_in_tumour = stroma_map * self.bulk_mask_np
        #Get what is tumour contained in bulk
        tumor_in_bulk = tumor_map * self.bulk_mask_np

        #Tumour stroma ratio = stroma in tumour / stroma in tumour + tumour in bulk
        ...
        header = 'case_id,tsr_ratio\n'

        ho = open(output_csv_file, 'w')
        ho.write(header)
        ho.write('{case_id},{tsr_ratio}'.format(
            case_id=series, 
            tsr_ratio=tsr))
        ho.close()
...
if __name__ == "__main__":
    ...
    # compute tumor-stroma ratio heatmap
